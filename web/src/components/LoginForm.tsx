...
export default function LoginForm() {
  ...
  return (
    <div className="mx-auto mt-10 max-w-sm rounded-xl border border-[#E6DBC5] dark:border-[#2b303c] bg-white dark:bg-[#1c1f26] p-6 shadow-sm">
      <h2 className="text-xl font-semibold text-[#141E46] dark:text-white">Sign in</h2>
      <p className="mt-1 text-sm text-[#5A6578] dark:text-slate-400">Welcome back to the employee directory.</p>
      ...
      <form onSubmit={handleLogin} className="mt-6 space-y-4">
        <div>
          <label htmlFor="login-username" className="block text-sm font-medium text-[#141E46] dark:text-slate-300">Username</label>
          <input
            id="login-username"
            type="text"
            required
        ...
        <div>
          <label htmlFor="login-password" className="block text-sm font-medium text-[#141E46] dark:text-slate-300">Password</label>
          <input
            id="login-password"
            type="password"
            required
      ...
      <div className="mt-4 text-center text-sm">
        <a href="/register" className="text-[#5A6578] dark:text-slate-400 hover:text-[#41B06E] dark:hover:text-[#1DCD9F] transition-colors">Don't have an account? Register</a>
      </div>
    </div>
  );
}
